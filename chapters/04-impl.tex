...
handles. Upon publication of a requested topic, the pending subscription will
be completed with \lstinline{}
}]
...
impl RequestHandler {
    ...
    -> Box<Future<Item=Topic, Error=SubscribeError>>
  {
    ...
    if let Some(topic) = self.catalog.lookup(&name) {
      ...
      // return a sucessfully resolved future for the topic
      ...
      let (handle, future) = promise();
      // insert it into the list of pending lookups
      self.lookups.entry(name).or_insert(vec![]).push(handle);
